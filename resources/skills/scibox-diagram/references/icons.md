...
data = pathlib.Path('assets/icons/tabler/outline/database.svg').read_bytes()
svg = data[data.index(b'<svg'):]                    # 剥掉 <svg> 前的注释头
uri = 'data:image/svg+xml,' + base64.b64encode(svg).decode()
style = f'shape=image;html=1;imageAspect=1;image={uri};'
```

两个坑：
1. **URI 里不能写 `;base64`**——drawio 的 style 串是分号分隔的，`base64,xxx` 会被当成
   另一个 style 键，图标渲染成裂图占位框。约定是 `data:image/svg+xml,<base64>`（逗号直连）。
2. **图标所在位置不要压住任何实线顶点**（如标题条）：把标题条缩窄、图标放旁边，
   否则 `check_layout` 报重叠 FAIL。
