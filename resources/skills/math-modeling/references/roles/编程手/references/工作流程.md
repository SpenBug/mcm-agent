...
6. 生成建模流程图（**必须**，`flow_overall_model` / `flow_qN_model`）：按 `tools/figure/references/chart-types/flowchart.md` 执行，至少 1 幅总体图，子问题存在独立分支、循环或三步以上链时再加子问题图，节点与连线来自真实模型/代码，用已安装的 Matplotlib/MATLAB 生成并保留可运行代码；流程图不计入三类候选图配额，也不替代结果图。
...
代码、结果表、每类至少 3 张且覆盖全部子问题的三类图和复现清单完成作者自检并冻结后，按 `../../../../references/Subagent调度.md` 派发独立质检 Subagent。质检者重跑唯一复现命令，核对哈希、环境、种子、参数、关键数值、约束、边界、量纲、统计计算和各子问题图表语义；带全部 `--questions` 的 `figure_audit.py --strict` 结果和实际 PNG/灰度预览作为证据输入；并核对建模流程图与真实模型/代码一致、覆盖全部子问题且未替代结果图。`P2` 未返回 `PASS` 不得进入论文阶段。
